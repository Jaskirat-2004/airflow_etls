"""
PROGRAMMER : JASKIRAT
INFO : LEAPMAX ETL FOR DATA MIGRATION (TIME_SERIES - activity, FULL_REFRESH - rest)
"""
...
from leapmax.config.leapmax_config import (
    ENABLED_TABLES,
    TABLES_CONFIG,
    TENANT_DB_MAP,
    TENANTS
...
def handle_time_series(source_hook, dest_cursor, table:str, tenant:str, table_config:dict):
    ...
    temp_conn = source_hook.get_conn()
    temp_cursor = temp_conn.cursor()

    # ==================== GET COLUMN LIST ====================
    temp_cursor.execute(f'SELECT * FROM "{table}" LIMIT 0')
    ...
    while window_start < final_end :
        ...
        try:
            ...
            # ===== FRESH SOURCE CONNECTION PER WINDOW =====
            ...
            dest_cursor.connection.rollback()
            logger.error(f"J===== WINDOW FAILED | [{table}] | [{tenant}] | ERROR: {e} =====S")
            raise
        else:
            dest_cursor.connection.commit()
            logger.info(f"J===== WINDOW COMMITED | [{table}] | [{tenant}] =====S")
        finally:
...
def leapmax_etl():
    ...
    logger.info("J===== STARTING LEAPMAX ETL =====S")
    ...
    def migrate_tables(table: str, tenant: str):
        ...
        try:
            ...
            # For time_series reopen connection per window
            if strategy == "full_refresh":
                source_conn = source_hook.get_conn()
                source_cursor = source_conn.cursor()
            ...
            try:
                if strategy == "full_refresh":
                    ...
                    handle_time_series(source_hook, dest_cursor, table, tenant, table_config)
