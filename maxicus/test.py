...
def get_last(dest_cursor, table:str, tenant:str):
    ...
    try:
        ...
        if result:
            ...
            total_rows = int(result[1]) if result and result[1] is not None else 0
        ...
        logger.info(f"J===== LAST RUN FOR [{table}] | [{tenant}] : [{last_run}] | [{total_rows}] =====S")

    last_run = last_run.strftime("%Y-%m-%d %H:%M:%S")
    return last_run,total_rows
    
def update_last(dest_cursor, table:str, tenant:str, last_run, total_rows:int):

    query = f"""
    INSERT INTO leap_tracking (table_name,tenant_name,last_run,total_rows)
    VALUES (%s,%s,%s,%s)
    ...
    try:
        dest_cursor.execute(query, (table, tenant, last_run, total_rows))
        ...
        logger.info(f"J===== LAST RUN FOR [{table}] | [{tenant}] UPDATED SUCCESSFULLY TO [{last_run}] | [{total_rows}] =====S")
...
def handle_append(source_cursor, dest_cursor, table:str, tenant:str, table_config:dict):
    ...
    window_interval = 180 # IN MINUTES
    ...
    while window_start < final_end :
        ...
        update_last(dest_cursor, table, tenant, window_start, 0)
    # ==================== UPDATE LAST RUN IN TRACKING TABLE ====================
    count_query = f"""
    SELECT COUNT(*) FROM "{table}"
    """
    dest_cursor.execute(count_query)
    count = dest_cursor.fetchone()[0]
    update_last(dest_cursor, table, tenant, window_start, count)
    logger.info(f"J===== TRACKING TABLE UPDATED FOR [{table}] | [{tenant}] | FINAL LAST RUN : [{window_start}] | TOTAL ROWS : [{count}] =====S")
...
def handle_upsert(source_cursor, dest_cursor, table:str, tenant:str, table_config:dict):
    ...
    while True:
        ...
        dest_cursor.execute(f"""
            SELECT MAX("{incremental_column}") FROM "{staging_table}"
        """)
        ...
        update_last(dest_cursor, table, tenant, last_run, 0)
    # ==================== UPDATE LAST RUN IN TRACKING TABLE ====================
    count_query = f"""
    SELECT COUNT(*) FROM "{table}"
    """
    dest_cursor.execute(count_query)
    count = dest_cursor.fetchone()[0]
    update_last(dest_cursor, table, tenant, last_run, count)
    logger.info(f"J===== TRACKING TABLE UPDATED FOR [{table}] | [{tenant}] | FINAL LAST RUN : [{last_run}] | TOTAL ROWS : [{count}] =====S")
...
def handle_full_refresh(source_cursor, dest_cursor, table:str, tenant:str, table_config:dict):
    ...
    dest_cursor.execute(count_query)
    count = dest_cursor.fetchone()[0]
    update_last(dest_cursor, table, tenant, pendulum.now(), count)
    logger.info(f"J===== FULL REFRESH DONE: [{table}] | [{tenant}] | TOTAL ROWS : [{count}] =====S")
