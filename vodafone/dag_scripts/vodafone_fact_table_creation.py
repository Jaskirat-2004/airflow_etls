"""
PROGRAMMER : JASKIRAT
INFO : VODAFONE - CREATE CLICKHOUSE FACT TABLES (idempotent, explicit DDL)
"""
...
from vodafone.config.fact_table_config import JS_FACT_TABLES_CREATE, JS_FACT_CONFIG
...
@dag(
    dag_id="vodafone_create_fact_tables",
    start_date=pendulum.datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    tags=["vodafone", "fact", "creation","ddl"],
    default_args=default_args,
)
def vodafone_create_fact_tables():
    
    logger.info("JS ====== DAG STARTED : [vodafone_fact_table_creation] ===== JS")
