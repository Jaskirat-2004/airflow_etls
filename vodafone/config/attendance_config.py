# JS =============================================================================================================== JS
#                                       QUERIES FOR ATTENDANCE REPORT FACT TABLE
# JS =============================================================================================================== JS
...
VODAFONE_FACT_ATTENDANCE_REPORT_DDL = """

CREATE TABLE IF NOT EXISTS vodafone.vodafone_fact_attendance_report
(
    report_date            Date,
    week                   String,
    emp_id                 String,
    ...
    primary_campaign       Nullable(String),
...
VODAFONE_FACT_ATTENDANCE_REPORT_QUERY = """

WITH
ros AS (
    SELECT report_date, emp_id, roster AS shift_raw,
        ...
        CASE WHEN roster ~ '^[0-9]{{2}}:[0-9]{{2}}' THEN 'SHIFT'
             WHEN upper(roster)='WO' THEN 'WO' WHEN roster='Leave' THEN 'Leave' ELSE upper(roster) END AS status
    FROM roster
    WHERE report_date > '{last_processed}'
),
ap_iv AS (
    SELECT report_date, split_part(user_id,'@',1) AS emp_id, interval_start::time AS iv,
...
ap AS (
    SELECT a.report_date, a.emp_id,
        SUM(a.rs) AS raw_ready, SUM(a.bs) AS break_secs,
...
ses AS (
    SELECT report_date, split_part(user_id,'@',1) AS emp_id, MIN(login_time) AS first_login, MAX(logout_time) AS last_logout
    FROM agent_session_details
    WHERE report_date > '{last_processed}'
    GROUP BY 1,2
),
dt AS (
    SELECT report_date, emp_id,
...
base AS (
    SELECT
        h.report_date, h.employee_id AS emp_id, h.employee_name AS emp_name,
        h.designation_name AS designation, h.grade, h.employee_type,
        h.reporting_to_name AS team_leader, h.functional_reporting_to_name AS functional_manager,
        h.location_name AS location, h.ou_name, h.department_name AS department,
        h.gender, h.current_status, h.workplace_category AS wfo_wfh, h.date_of_joining AS doj,
        GREATEST((h.report_date - h.date_of_joining), 0) AS tenure_days,
        COALESCE(r.status,'NO-ROSTER') AS roster_status, r.shift_raw, r.shift_start, r.shift_end, r.ss, r.se,
        ...
        COALESCE(ap.raw_ready,0) AS raw_ready_secs, COALESCE(ap.break_secs,0) AS break_secs,
        COALESCE(ap.inshift_ready,0) AS inshift_ready_secs, COALESCE(ap.inshift_ready_buf,0) AS inshift_ready_buf_secs,
        ...
        ac.primary_campaign, ac.primary_process
    FROM vodafone_head_count h
    LEFT JOIN ros r      ON r.report_date  = h.report_date AND r.emp_id  = h.employee_id
    LEFT JOIN ap         ON ap.report_date = h.report_date AND ap.emp_id = h.employee_id
    LEFT JOIN ap_camp ac ON ac.report_date = h.report_date AND ac.emp_id = h.employee_id
    LEFT JOIN ses s      ON s.report_date  = h.report_date AND s.emp_id  = h.employee_id
    LEFT JOIN dt         ON dt.report_date = h.report_date AND dt.emp_id = h.employee_id
    WHERE h.report_date > '{last_processed}'
)
SELECT
    report_date,
    'Week-' || CEIL(EXTRACT(DAY FROM report_date)/7.0)::int          AS week,
    emp_id, emp_name, 'vodafone' AS tenant_name, 'maxicus' AS entity_name,
    COALESCE(ou_name,'Vodafone') AS process, primary_campaign,
    CASE WHEN primary_campaign = 'Chat_In' THEN 'Inbound' WHEN primary_campaign IS NULL THEN NULL ELSE 'Outbound' END AS call_type,
    designation, grade, employee_type, team_leader, functional_manager, location, ou_name, department,
    gender, current_status, wfo_wfh, doj, tenure_days,
    ...
    shift_raw, roster_status, shift_start, shift_end, scheduled_secs,
    ...
    raw_ready_secs, break_secs, exception_secs, adjusted_ready_secs, inshift_ready_secs, inshift_ready_buf_secs,
