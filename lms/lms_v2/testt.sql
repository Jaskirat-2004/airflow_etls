WITH course_linked_submissions AS (
    SELECT s.*
    FROM "Submission" s
    INNER JOIN "courses" c
        ON c."id"           = s."courseId"
        AND c."tenant_name" = s."tenant_name"
...
FROM course_linked_submissions s
...
--   ===========================================================================
--   
--   THIS ONE RETURNS HAS NO LMS LINK THE COURSE IDS ARE NULL IN THE ABOVE SO NO DTA RETURNED ON JOIN
...
WITH submissions AS (
    ...
    FROM "Submission"
    WHERE "isSubmitted" = 1
      AND DATE("submittedAt") >= CURRENT_DATE - INTERVAL '90 days'
),
